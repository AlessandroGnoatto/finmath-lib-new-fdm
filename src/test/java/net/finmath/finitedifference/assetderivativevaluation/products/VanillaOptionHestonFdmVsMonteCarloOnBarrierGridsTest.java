...
/**
 * Vanilla-only Heston finite-difference vs Monte Carlo diagnostic test.
 *
 * <p>
 * The purpose of this test is to check whether the plain vanilla Heston PDE solver
 * is already biased on the same spot domains used by the barrier tests.
 * </p>
 *
 * <p>
 ...
 * </p>
 * <ul>
 *   <li>call on spot grid [0, 120],</li>
 *   <li>put on spot grid [80, 300].</li>
 * </ul>
 */
public class VanillaOptionHestonFdmVsMonteCarloOnBarrierGridsTest {
	...
	private static final double CALL_GRID_MIN = 0.0;
	private static final double CALL_GRID_MAX = 120.0;
	private static final int CALL_GRID_STEPS = 240;   // gives deltaS = 0.5, same as up-barrier tests

	private static final double PUT_GRID_MIN = 80.0;
	private static final double PUT_GRID_MAX = 300.0;
	private static final int PUT_GRID_STEPS = 440;    // gives deltaS = 0.5, same as down-barrier tests

	private static final int MC_NUMBER_OF_TIME_STEPS = 500;
	private static final int MC_NUMBER_OF_PATHS = 50_000;
	private static final int SEED = 31415;

	@Test
	public void testVanillaCallOnUpBarrierLikeGridHestonFiniteDifferenceVsMonteCarlo() throws Exception {
		runVanillaTest(
				CallOrPut.CALL,
				new UniformGrid(CALL_GRID_STEPS, CALL_GRID_MIN, CALL_GRID_MAX)
		);
	}

	@Test
	public void testVanillaPutOnDownBarrierLikeGridHestonFiniteDifferenceVsMonteCarlo() throws Exception {
		runVanillaTest(
				CallOrPut.PUT,
				new UniformGrid(PUT_GRID_STEPS, PUT_GRID_MIN, PUT_GRID_MAX)
		);
	}

	private void runVanillaTest(final CallOrPut callOrPut, final Grid sGrid) throws Exception {
		...
		final EuropeanOption fdmProduct = new EuropeanOption(
				MATURITY,
				STRIKE,
				callOrPut
		);
		...
		if(callOrPut == CallOrPut.CALL) {
			final net.finmath.montecarlo.assetderivativevaluation.products.EuropeanOption mcCall =
					new net.finmath.montecarlo.assetderivativevaluation.products.EuropeanOption(
							MATURITY,
							STRIKE
					);

			mcPrice = mcCall.getValue(mcSimulation);
		}
		else {
			final net.finmath.montecarlo.assetderivativevaluation.products.EuropeanOption mcCall =
					new net.finmath.montecarlo.assetderivativevaluation.products.EuropeanOption(
			...
			final double mcCallPrice = mcCall.getValue(mcSimulation);

			final double discountedStrike = STRIKE * Math.exp(-R * MATURITY);
			final double discountedSpot = S0 * Math.exp(-Q * MATURITY);

			mcPrice = mcCallPrice - discountedSpot + discountedStrike;
		}
		final double absoluteError = Math.abs(fdPrice - mcPrice);
		final double relativeError = absoluteError / Math.max(Math.abs(mcPrice), 1E-8);

		System.out.println("====================================");
		System.out.println("Vanilla case    = " + callOrPut);
