...
/**
 * Tests for {@link BestOfOption}.
 *
 * @author Alessandro Gnoatto
 */
public class BestOfOptionTest {
	...
	private static final double LOWER_BOUND_TOLERANCE = 1.0E-10;
	...
	public void testSymmetryUnderAssetSwap() {
		...
		final BestOfOption originalOption = new BestOfOption(
				"Asset1",
				"Asset2",
				MATURITY,
				strike,
		...
		final BestOfOption swappedOption = new BestOfOption(
				"Asset2",
				"Asset1",
				MATURITY,
				strike,
	...
	@Test
	public void testBestOfCallDominatesSingleAssetCalls() {
		final double strike = 100.0;

		final BestOfOption bestOfOption = new BestOfOption(
				"Asset1",
				"Asset2",
				MATURITY,
				strike,
		...
		final FDMMultiAssetBlackScholesModel twoAssetModel = createTwoAssetPdeModel(
				SPOT_1,
				SPOT_2,
				RISK_FREE_RATE,
				DIVIDEND_YIELD_1,
		...
		final double bestOfPrice = interpolateAtInitialState(
				bestOfOption.getValue(0.0, twoAssetModel),
				twoAssetModel
		);

		final EuropeanOption callOnFirstAsset = new EuropeanOption(
				"Asset1",
				MATURITY,
				strike,
				CallOrPut.CALL
		...
		final FDMBlackScholesModel firstAssetModel = createOneAssetModel(
				SPOT_1,
				RISK_FREE_RATE,
				DIVIDEND_YIELD_1,
				VOLATILITY_1
		...
		final double firstAssetCallPrice = interpolateAtSpot(
				callOnFirstAsset.getValue(0.0, firstAssetModel),
				firstAssetModel.getSpaceTimeDiscretization().getSpaceGrid(0).getGrid(),
				SPOT_1
		);

		final EuropeanOption callOnSecondAsset = new EuropeanOption(
				"Asset2",
				MATURITY,
				strike,
				CallOrPut.CALL
		...
		final FDMBlackScholesModel secondAssetModel = createOneAssetModel(
				SPOT_2,
				RISK_FREE_RATE,
				DIVIDEND_YIELD_2,
				VOLATILITY_2
		...
		final double secondAssetCallPrice = interpolateAtSpot(
				callOnSecondAsset.getValue(0.0, secondAssetModel),
				secondAssetModel.getSpaceTimeDiscretization().getSpaceGrid(0).getGrid(),
				SPOT_2
		);

		assertTrue(bestOfPrice + LOWER_BOUND_TOLERANCE >= firstAssetCallPrice);
		assertTrue(bestOfPrice + LOWER_BOUND_TOLERANCE >= secondAssetCallPrice);
	}
	...
	public void testBestOfCallAgainstMonteCarlo() throws CalculationException {
		...
		final BestOfOption pdeProduct = new BestOfOption(
				"Asset1",
				"Asset2",
				MATURITY,
				strike,
		...
		final MonteCarloEstimate monteCarloEstimate = getMonteCarloBestOfValue(
				monteCarloModel,
				MATURITY,
				strike,
				CallOrPut.CALL
	...
	private static MonteCarloEstimate getMonteCarloBestOfValue(
		...
		final RandomVariable bestOf = asset1.sub(asset2).choose(asset1, asset2);
		final RandomVariable payoff = bestOf.sub(strike).mult(callOrPut.toInteger()).floor(0.0);
