...
/**
 * Focused debug test for the direct 2D Heston down-in put barrier solve.
 *
 * <p>
 * The aim is diagnostic: identify where strong negative values occur on the
 * grid and inspect a local neighborhood around the minimum.
 * </p>
 */
public class BarrierOptionHestonDownInPutDirectDebugTest {
	...
	@Test
	public void testDownInEuropeanPutDirectHestonDebug() throws Exception {
		...
		System.out.println("HESTON DIRECT DOWN-IN PUT DEBUG");
		...
		printNeighborhood(knockInValuesOnGrid, sNodes, vNodes, nS, nV, minIS, minIV, 4, 1);

		printSpotSlice(knockInValuesOnGrid, sNodes, vNodes, nS, 0, "Slice at v=0");
		if(nV > 1) {
			printSpotSlice(knockInValuesOnGrid, sNodes, vNodes, nS, 1, "Slice at first positive v");
		}
		printSpotSlice(knockInValuesOnGrid, sNodes, vNodes, nS, v0Index, "Slice at v closest to v0");
	...
	private static void printNeighborhood(
		...
		for(int j = vStart; j <= vEnd; j++) {
			...
			for(int i = sStart; i <= sEnd; i++) {
				final double value = values[i + j * nS];
				row.append("[").append(i).append(": S=").append(sNodes[i]).append(", V=").append(value).append("] ");
			}
	...
	private static void printSpotSlice(
		...
		for(int i = 0; i < nS; i += stride) {
			final double value = values[i + vIndex * nS];
			row.append("[").append(i).append(": S=").append(sNodes[i]).append(", V=").append(value).append("] ");
		}
