...
/**
 * Focused debug test for the direct 2D Heston down-in put case,
 * targeting the new negative pocket at the upper variance boundary.
 */
public class BarrierOptionHestonDownInPutUpperVarianceDebugTest {
	...
	@Test
	public void testDownInEuropeanPutUpperVarianceDebug() throws Exception {
		...
		final double[] vanillaSliceAtInitialVariance =
				extractSliceAtSecondStateIndex(vanillaValuesOnGrid, nS, nV, v0Index);
		final double[] knockInSliceAtInitialVariance =
				extractSliceAtSecondStateIndex(knockInValuesOnGrid, nS, nV, v0Index);
		...
		System.out.println("HESTON DIRECT DOWN-IN PUT UPPER-V DEBUG");
		...
		System.out.println("Top variance index          = " + (nV - 1) + ", v = " + vNodes[nV - 1]);
		System.out.println("Penultimate variance index  = " + (nV - 2) + ", v = " + vNodes[nV - 2]);
		System.out.println("Initial variance index      = " + v0Index + ", v = " + vNodes[v0Index]);

		if(barrierIndex >= 0) {
			printVarianceColumnAroundBarrier(knockInValuesOnGrid, sNodes, vNodes, nS, nV, barrierIndex, 8);
			printVarianceColumnAroundBarrier(vanillaValuesOnGrid, sNodes, vNodes, nS, nV, barrierIndex, 8);
		}

		printSpotSlice(knockInValuesOnGrid, sNodes, vNodes, nS, nV - 1, "Knock-in slice at top v");
		printSpotSlice(knockInValuesOnGrid, sNodes, vNodes, nS, nV - 2, "Knock-in slice at penultimate v");
		printSpotSlice(knockInValuesOnGrid, sNodes, vNodes, nS, v0Index, "Knock-in slice at v closest to v0");

		printSpotSlice(vanillaValuesOnGrid, sNodes, vNodes, nS, nV - 1, "Vanilla slice at top v");
		printSpotSlice(vanillaValuesOnGrid, sNodes, vNodes, nS, nV - 2, "Vanilla slice at penultimate v");
		printSpotSlice(vanillaValuesOnGrid, sNodes, vNodes, nS, v0Index, "Vanilla slice at v closest to v0");

		printNeighborhood(knockInValuesOnGrid, sNodes, vNodes, nS, nV, minIS, minIV, 5, 2);
	...
	private static void printVarianceColumnAroundBarrier(
			final double[] values,
			final double[] sNodes,
			final double[] vNodes,
			final int nS,
		...
		System.out.println("Variance column at barrier-near spot: iS=" + iS + ", S=" + sNodes[iS]);
		final StringBuilder row = new StringBuilder("  ");
		for(int j = 0; j < nV; j += Math.max(1, stride)) {
			row.append("[")
				.append(j)
				.append(": v=").append(vNodes[j])
				.append(", V=").append(values[iS + j * nS])
				.append("] ");
		}
		if((nV - 1) % Math.max(1, stride) != 0) {
			final int j = nV - 1;
			row.append("[")
				.append(j)
				.append(": v=").append(vNodes[j])
