...
/**
 * Tests for {@link DigitalBasketOption}.
 *
 * <p>
 * Since finmath does not yet provide a dedicated Monte Carlo digital basket
 * product, the Monte Carlo payoff is constructed inline from the simulated
 * asset values.
 * </p>
 *
 * <p>
 * The test class covers the three payoff families supported by
 ...
 */
public class DigitalBasketOptionTest {
	...
	private static final int NUMBER_OF_PATHS_MC = 100000;
	private static final int RANDOM_SEED = 3141;

	private static final double CASH_PAYOFF = 10.0;

	private static final double SYMMETRY_TOLERANCE = 2.0E-3;
	private static final double PDE_DISCRETIZATION_ALLOWANCE = 6.0E-1;
	private static final double MONTE_CARLO_SIGMA_MULTIPLIER = 4.0;

	@Test
	public void testLinearCombinationAgainstMonteCarlo() throws CalculationException {
		final double[] quantities = new double[] { 1.0, -1.0 };
		final double strike = 5.0;

		final DigitalBasketOption pdeProduct = DigitalBasketOption.linearCombination(
		...
		final MonteCarloEstimate monteCarloEstimate = getMonteCarloDigitalBasketValue(
				monteCarloModel,
				MATURITY,
				DigitalBasketOption.BasketDigitalType.LINEAR_COMBINATION,
				quantities,
				strike,
				CASH_PAYOFF,
				CallOrPut.CALL
		...
		assertTrue(
				"Linear-combination digital: PDE price " + pdePrice + " differs from Monte Carlo price "
	...
	@Test
	public void testBestOfCallAgainstMonteCarlo() throws CalculationException {
		final double strike = 100.0;

		final DigitalBasketOption pdeProduct = DigitalBasketOption.bestOf(
				new String[] { "Asset1", "Asset2" },
				MATURITY,
				strike,
				CASH_PAYOFF,
		...
		final MonteCarloEstimate monteCarloEstimate = getMonteCarloDigitalBasketValue(
				monteCarloModel,
				MATURITY,
				DigitalBasketOption.BasketDigitalType.BEST_OF,
				null,
				strike,
				CASH_PAYOFF,
				CallOrPut.CALL
		...
		assertTrue(
				"Best-of digital call: PDE price " + pdePrice + " differs from Monte Carlo price "
						+ monteCarloEstimate.getValue() + " by more than tolerance " + tolerance,
				Math.abs(pdePrice - monteCarloEstimate.getValue()) <= tolerance
		);
	}
	...
	@Test
	public void testBestOfPutAgainstMonteCarlo() throws CalculationException {
		final double strike = 100.0;
		...
		assertTrue(
				"Best-of digital put: PDE price " + pdePrice + " differs from Monte Carlo price "
						+ monteCarloEstimate.getValue() + " by more than tolerance " + tolerance,
				Math.abs(pdePrice - monteCarloEstimate.getValue()) <= tolerance
		);
	}
	...
	public void testWorstOfCallAgainstMonteCarlo() throws CalculationException {
		...
		final DigitalBasketOption pdeProduct = DigitalBasketOption.worstOf(
				new String[] { "Asset1", "Asset2" },
				MATURITY,
				strike,
				CASH_PAYOFF,
		...
		final MonteCarloEstimate monteCarloEstimate = getMonteCarloDigitalBasketValue(
				monteCarloModel,
				MATURITY,
				DigitalBasketOption.BasketDigitalType.WORST_OF,
				null,
				strike,
				CASH_PAYOFF,
				CallOrPut.CALL
		...
		assertTrue(
				"Worst-of digital call: PDE price " + pdePrice + " differs from Monte Carlo price "
						+ monteCarloEstimate.getValue() + " by more than tolerance " + tolerance,
				Math.abs(pdePrice - monteCarloEstimate.getValue()) <= tolerance
		);
	}
	...
	public void testWorstOfPutAgainstMonteCarlo() throws CalculationException {
		...
		assertTrue(
				"Worst-of digital put: PDE price " + pdePrice + " differs from Monte Carlo price "
						+ monteCarloEstimate.getValue() + " by more than tolerance " + tolerance,
				Math.abs(pdePrice - monteCarloEstimate.getValue()) <= tolerance
		);
	}
	...
	@Test
	public void testBestOfSymmetryUnderAssetSwap() {
		final double strike = 100.0;

		final DigitalBasketOption originalOption = DigitalBasketOption.bestOf(
				new String[] { "Asset1", "Asset2" },
				MATURITY,
				strike,
				CASH_PAYOFF,
		...
		final DigitalBasketOption swappedOption = DigitalBasketOption.bestOf(
	...
	@Test
	public void testWorstOfSymmetryUnderAssetSwap() {
		final double strike = 100.0;

		final DigitalBasketOption originalOption = DigitalBasketOption.worstOf(
				new String[] { "Asset1", "Asset2" },
				MATURITY,
				strike,
				CASH_PAYOFF,
		...
		final DigitalBasketOption swappedOption = DigitalBasketOption.worstOf(
				new String[] { "Asset2", "Asset1" },
				MATURITY,
				strike,
				CASH_PAYOFF,
	...
	private static MonteCarloEstimate getMonteCarloDigitalBasketValue(
			final MonteCarloMultiAssetBlackScholesModel model,
			final double maturity,
			final DigitalBasketOption.BasketDigitalType basketDigitalType,
		...
		final RandomVariable underlying;

		switch(basketDigitalType) {
		case LINEAR_COMBINATION:
			underlying = asset1.mult(quantities[0]).add(asset2.mult(quantities[1]));
			break;
		case BEST_OF:
			underlying = asset1.sub(asset2).choose(asset1, asset2);
			break;
		case WORST_OF:
			underlying = asset1.sub(asset2).choose(asset2, asset1);
			break;
		default:
			throw new IllegalStateException("Unsupported basket digital type.");
		}

		final RandomVariable zero = asset1.mult(0.0);
		final RandomVariable signal = underlying.sub(strike).mult(callOrPut.toInteger());
		final RandomVariable payoff = signal.choose(zero.add(cashPayoff), zero);
