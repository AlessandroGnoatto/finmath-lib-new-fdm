...
/**
 * Regression tests for 2D discretely monitored double knock-out options
 * under Heston and SABR.
 */
public class DoubleBarrierOptionStochasticVolatilityDiscreteMonitoringKnockOutTest {
	...
	public void testHestonEuropeanDiscreteMonitoringOrderingAgainstContinuousAndVanilla() {
		...
		assertTrue(vanillaValue + ORDERING_TOLERANCE_2D >= coarseValue);
		assertTrue(coarseValue + ORDERING_TOLERANCE_2D >= fineValue);
		assertTrue(fineValue + ORDERING_TOLERANCE_2D >= continuousValue);
		assertTrue(continuousValue >= -ORDERING_TOLERANCE_2D);
	}
	...
	@Test
	public void testHestonBermudanAmericanOrderingUnderDiscreteMonitoring() {
	...
	@Test
	public void testSabrBermudanAmericanOrderingUnderDiscreteMonitoring() {
	...
	@Test
	public void testHestonDiscreteMonitoringZeroesOutsideBandAtMonitoringTime() {

		final double monitoringTime = 0.50;
		final FDMHestonModel model = createHestonModel();

		final DoubleBarrierOption discreteKnockOut = new DoubleBarrierOption(
				null,
				MATURITY,
				STRIKE,
				LOWER_BARRIER,
		...
		for(int j = 0; j < x1.length; j++) {
			for(int i = 0; i < x0.length; i++) {
				if(x0[i] <= LOWER_BARRIER + GRID_TOLERANCE || x0[i] >= UPPER_BARRIER - GRID_TOLERANCE) {
					assertEquals(0.0, valuesAtMonitoring[flatten(i, j, x0.length)], ZERO_TOLERANCE);
				}
			}
		}
	}
	...
	@Test
	public void testSabrDiscreteMonitoringZeroesOutsideBandAtMonitoringTime() {
