...
/**
 * Regression tests for 1D discretely monitored double knock-out options.
 */
public class DoubleBarrierOptionDiscreteMonitoringKnockOutTest {
	...
	public void testEuropeanCallDiscreteMonitoringOrderingAgainstContinuousAndVanilla() {
		...
		assertTrue(fineValue + ORDERING_TOLERANCE >= continuousValue);
		assertTrue(continuousValue >= -ORDERING_TOLERANCE);
	}
	...
	@Test
	public void testDiscreteMonitoringZeroesOutsideBandAtMonitoringTime() {
		...
		for(int i = 0; i < spotGrid.length; i++) {
			if(spotGrid[i] <= LOWER_BARRIER + GRID_TOLERANCE || spotGrid[i] >= UPPER_BARRIER - GRID_TOLERANCE) {
				assertEquals(0.0, valuesAtMonitoring[i], ZERO_TOLERANCE);
			}
		}
	}
