...
/**
 * Slow structural tests for the 2D shout recursion.
 *
 * Focus:
 * - 2 shouts >= 1 shout
 * - 1 shout >= immediate-reset vanilla
 *
 * These tests still use reduced grids compared with the original all-in-one class,
 * but they are intentionally heavier than the smoke suite.
 */
public class ShoutOption2DStructuralSlowTest {
	...
	private static final int NUMBER_OF_TIME_STEPS = 50;
	private static final int NUMBER_OF_SPACE_STEPS_S = 90;
	private static final int NUMBER_OF_SPACE_STEPS_SECOND = 35;

	private static final double SPACE_MIN = 20.0;
	private static final double SPACE_MAX = 200.0;

	private static final double ORDERING_TOL_2D = 1.5E-1;
	private static final double RESET_DOMINANCE_TOL_2D = 2.5E-1;

	@Test
	public void testHestonTwoShoutsDominateOneShoutPut() {
		runTwoShoutsDominateOneShoutTest(ModelType.HESTON);
	}

	@Test
	public void testSabrTwoShoutsDominateOneShoutPut() {
		runTwoShoutsDominateOneShoutTest(ModelType.SABR);
	}

	@Test
	public void testHestonOneShoutDominatesImmediateResetVanillaPut() {
		runImmediateResetDominanceTest(ModelType.HESTON);
	}

	@Test
	public void testSabrOneShoutDominatesImmediateResetVanillaPut() {
		runImmediateResetDominanceTest(ModelType.SABR);
	}

	private void runTwoShoutsDominateOneShoutTest(final ModelType modelType) {
		...
		final double oneShoutValue = interpolate2DAtInitialState(
		...
		final double twoShoutValue = interpolate2DAtInitialState(
		...
		assertTrue(twoShoutValue + ORDERING_TOL_2D >= oneShoutValue);
	}

	private void runImmediateResetDominanceTest(final ModelType modelType) {
		final TwoDimensionalSetup setup = createTwoDimensionalSetup(modelType, SPOT_HIGH);
		...
		final double resetVanillaValue = interpolate2DAtInitialState(
		...
		assertTrue(shoutValue + RESET_DOMINANCE_TOL_2D >= resetVanillaValue);
	}

	private double[] buildSlowStrikeGrid() {
		final int n = 31;
